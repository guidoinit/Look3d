...
l3d_modifier_spin::l3d_modifier_spin()
{
    _mesh=l3d_null;
    _center.x=_center.y=_center.z=0.0F;

    _rotate.x=_rotate.y=_rotate.z=0.0F;
}
void l3d_modifier_spin::set_mesh(pl3d_mesh_struct _e)
{
    _mesh=_e;
}
void l3d_modifier_spin::spinedges_steps(l3d_uint _steps,l3d_vertex_fast _degree)
{
    for(l3d_uint ix=0; ix<_steps;ix++)
        spinedges(_degree);
}

void l3d_modifier_spin::spinedges()
{
    std::list<int>::iterator it;

    //1) estrudo i vertici selezionati
    extrudeedge();

    //2) i devo ruotare in base al vertice (0,0,0)

    for(it=_ladded.begin();it!=_ladded.end();++it)
    {
        pl3d_line_struct l=_mesh->_edges.find((*it));

        pl3d_vertex_fast v=l->_v0;
        //cambiare la dimenione dei vertici estrusi
        //sommare il world pos e center per ogni vertice

        *v+=_mesh->_transformation._wp;
        //*v+=_center;
        v->rotatez(_rotate.z);
        v->rotatey(_rotate.y);
        v->rotatex(_rotate.x);
...
void l3d_modifier_spin::spinedges(l3d_vertex_fast _degree)
{
    _rotate=_degree;
    spinedges();
}

void l3d_modifier_spin::spinvertex_steps(l3d_uint _steps,l3d_vertex_fast _degree)
{
...
void l3d_modifier_spin::spinvertex(l3d_vertex_fast _degree)
{
    _rotate=_degree;
    spinvertex();
}

void l3d_modifier_spin::spinvertex()
{
    std::list<int>::iterator it;
    ...
    for(it=_vadded.begin();it!=_vadded.end();++it)
    {
      ...
      //  glPushMatrix();
        ...
        //to-do: prima traslo o ruoto??
        //glTranslated(_mesh->_wp.x,_mesh->_wp.y,_mesh->_wp.z);
        //glTranslated(appx,appy,appz);
...
void l3d_modifier_spin::extrudevertex()
{
    if(_mesh==l3d_null)
        return;
    _vadded.clear();
...
void l3d_modifier_spin::extrudevertex(l3d_uint ix)
{
    pl3d_vertex_fast v=_mesh->_vertex.find(ix);
...
void l3d_modifier_spin::makeindex_quads()
{
...
void l3d_modifier_spin::makeindex_edges()
{
...
l3d_bool l3d_modifier_spin::isedge_perimeter(l3d_uint ix)
{
...
l3d_bool l3d_modifier_spin::isedgealone_sin(l3d_uint value)
{
...
l3d_bool l3d_modifier_spin::isedgealone_des(l3d_uint value)
{
    bool fls=false,fld=false;
...
l3d_bool l3d_modifier_spin::isquadalone(l3d_uint value)
{
    bool fl=false;
...
l3d_bool l3d_modifier_spin::isedgealone(l3d_uint value)
{
    bool fl=false;
...
l3d_bool l3d_modifier_spin::isquadall()
{
...
l3d_bool l3d_modifier_spin::isedgeall()
{
...
void l3d_modifier_spin::extrudeedge(std::list<int> _list)
{
...
void l3d_modifier_spin::extrudeedge_perimeter(std::list<int> _list)
{
...
void l3d_modifier_spin::extrudeedge(int l)
{
    int v1,v2,i1,i2;
...
void l3d_modifier_spin::extrudequad(l3d_uint iq)
{
...
void l3d_modifier_spin::extrudequad()
{
    std::list<int> qper;
    std::list<int>::iterator it;
...
std::list<int> l3d_modifier_spin::find_edges(l3d_uint il)
...
void l3d_modifier_spin::extrudeedge()
{
