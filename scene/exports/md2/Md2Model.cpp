...
texture * Md2Model::gettexture()
{
    setTexture(_filetex);
    return(_tex);
}

texture * Md2Model::gettexture(int it)
{

    if(it >_skinIds.size())
        return NULL;

     //_tex=_skinIds[it].second;

     return(_tex);
}
int Md2Model::getsizetex()
{
    return(_skinIds.size());
}
...
Md2Frame_t * Md2Model::getFrame(int iframe)
{

    return (&_frames[iframe]);

}
Md2Header_t * Md2Model::getHeader()
{
    return (&_header);
}
Md2Triangle_t * Md2Model::getTrianles()
{
    return(_triangles);
}

GLfloat Md2Model::getscale()
{
    return _scale;
}
